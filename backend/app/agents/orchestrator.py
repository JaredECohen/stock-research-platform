...
def _debate_outcome(m: StockMemoOut) -> str | None:
    ...
    """
    rec = m.debate
    if rec is None or rec.status == "not_run":
        return None
    # Before the hidden check: the presenter (B8-D4) hides the section of an
    # unavailable debate and shows its placeholder, and this sentence says no
    # more than that placeholder does.
    if rec.status == "unavailable":
        return "The bull/bear debate was unavailable for this memo."
    if memo_sections.is_hidden(m, "debate"):
        return None
