...
| GET | `/api/scorecard/spec` | none yet (Phase 6 F) | DB-only (`scorecard_versions`; in-code spec fallback) | public (auth off) | pro | data | Phase 6 C. Methodology + `spec_hash` |
...
| POST | `/api/admin/scorecard/refresh` | none | DB-only in-request (enqueues a `scorecard_runs` row; the worker's `scorecard_loop` scores at its next 03:45 UTC tick) | admin | admin | admin | Phase 6 C. 202; coalesces on `(version_key, as_of, kind)` |
