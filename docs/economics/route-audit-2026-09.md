...
| POST | `/api/admin/billing/overrides` | none | DB-only (`admin_overrides`; `trial_reset` also rewrites `users.trial_*` — an operator action, the one writer besides bootstrap) | admin | admin | admin | kinds `plan` (value `pro`), `quota` (recorded; not yet read by `entitlements.authorize` — see open issues), `trial_reset`, `suspend`; `reason` mandatory |
