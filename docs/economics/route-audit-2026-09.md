...
| GET | `/api/quotes` | `components/LiveQuote.tsx` on `pages/Research.tsx` and `pages/DCFLab.tsx` (`getQuotes`) | providers (`quote_service`: one FMP `/batch-quote` call, then at most 10 per-symbol quote calls in a 20 s budget; provider-cached under the calendar-aware 15-minute policy; DB-only `read_prices` close fallback; unknown tickers never reach a provider) | public | free | data | ≤50 tickers; 422 on malformed or empty; unknown ticker → `unavailable/unknown_ticker`, never 404; `Cache-Control: private, max-age=60`; not rendered on public sample pages |
| GET | `/api/stocks/{ticker}/memo` | `pages/Research.tsx` (`getStockMemo`; `components/AnalyzeStockGate.tsx` renders the 409 copy) | **LLM** (`run_stock_memo` runs inline when no snapshot or a stale one) + providers (`_ensure_lazy_universe` → profile lookup + 5-year `backfill_ticker`) | public | metered:memo_view | data (slowapi `memo_read` 60/min IP stays) | Under `AUTH_ENABLED`: never generates or backfills inline. No snapshot → 409 `no_memo` with `analyze_path`; stale snapshot served with `X-Memo-Stale: true` + reason; `ondemand=true` → routed through the analyze path (charged as `research_run`, 202); `as_of` → 404 `feature_disabled`. Free allowance counts **distinct tickers** per UTC month, not views. A stored snapshot that no longer validates (e.g. a mixed legacy bull_case list) → 422 `memo_unreadable` with `extra.{ticker,version,fields}`; the grant is released and the row is left as stored. |
| GET | `/api/stocks/{ticker}/memory` | `components/MemoryTrail.tsx` (`stockMemory`) | DB-only (learning ledger: `learning_items` + `learning_evidence`) when the effective learning mode is inject; otherwise DB/filesystem only (`memory/companies/<T>.md`) | public | pro | data | W7 (S19): the ledger branch never returns `detail` or a peer scope, labels every lesson a provisional hypothesis, and carries a coverage note |
...
| POST | `/api/admin/market-data/backfill` | operations | price and fundamental provider APIs; durable DB upserts; FMP-primary quarantine audits | admin | admin | admin | One planned ticker per request, atomic claim. `scope` is `all` or `fundamentals` (fundamentals skips prices); `dry_run=true` (fundamentals only, else 400) returns the exact FMP-primary plan and persists nothing, with no claim or cache change. Conflicting rows are quarantined with audits, never deleted; no filings, transcripts, LLMs, memo generation, evaluation or historical outcome writes. |
| GET | `/api/admin/market-data/fmp-repull` | operations | DB-only re-pull ledgers | admin | admin | admin | The worker's FMP re-pull dry-run and execute ledgers: per-ticker quarantine ids and reasons, adoptions, restatement values, relabels, entitlement denials, review list and result digest, plus the worker's own recorded environment view (never this web process's). No provider calls or writes. |
| POST | `/api/admin/market-data/fmp-repull/authorize` | operations | DB-only ledger row | admin | admin | admin | Owner authorization bound to the completed dry run's 64-hex result digest and the evidence-archive backup manifest's 64-hex SHA-256 (owner decision 3: back up before the re-pull), both required; creates the one-shot execute ledger the worker then runs. No provider call in the request; 404 before a dry run, 409 on a wrong digest, incomplete dry run or repeat, 422 without a valid backup SHA-256. |
...
| POST | `/api/admin/industries/jobs/recover-legacy` | operator after verified predecessor retirement | DB-only exact-state recovery of explicitly identified legacy jobs; no provider call or execution in-request | admin | admin | admin (slowapi `industry_admin` 10/min) | Requires 1–50 explicit job/run/attempt/start/heartbeat expectations and retirement evidence. Changed or owned jobs are refused, all dispositions retained, existing attempt caps and backoff preserved. A unique consistent published report can supply its exact receipt; publication is never inferred from the latest group report. |
| GET | `/api/admin/learning/status` | none | DB-only (grouped counts over `learning_items`, `learning_evidence`, `learning_renders`; the gates; last 10 `learning_control_events`) | admin | admin | admin | W7 (S18). Ceiling (`LEARNING_MODE_MAX`), DB mode, effective mode (min of the two; fails closed to off) |
| POST | `/api/admin/learning/mode` | none | DB-only (append control/status history: one `learning_control_events` row) | admin | admin | admin | W7 (S18). 409 `gates_not_met` for inject until G1-G4 are green unless `force` (recorded on the row); 422 for a bad mode or an empty reason. Reaches both services within the 60 s mode cache |
| GET | `/api/admin/learning/items` | none | DB-only (`learning_items` page ≤ 200 with posteriors and the latest 10 `learning_evidence` rows each) | admin | admin | admin | W7 (S18). Filters kind / scope_type / scope_key / status |
| POST | `/api/admin/learning/items/{item_id}/status` | none | DB-only (append control/status history: `learning_items.status_history`) | admin | admin | admin | W7 (S18). The per-lesson kill switch: active / suppressed / retired; 404 unknown item, 409 superseded |
| GET | `/api/admin/learning/renders` | none | DB-only (`learning_renders` page ≤ 200) | admin | admin | admin | W7 (S18). "Why did the agent see this?" — filter by ticker, run, memo snapshot, mode |
| GET | `/api/admin/learning/preview` | none | DB-only (active items in the ticker's scopes plus their evidence, and the renderer's bounded `block` for the consumer; writes nothing, not even a render row) | admin | admin | admin | W7 (S18; `block` S19). Evaluates priors without generating a memo |
| GET | `/api/admin/corpus-inventory` | none | DB-only (aggregate SQL; never selects embedding values) | admin | admin | admin | W7 (S17 census service, S18 route). Chunk classes, sources without chunks, repair plan with cost and storage |
...
| `/research` | `pages/Research.tsx` (+ `AnalyzeStockGate`, `MemoVersionTimeline`, `MemoryTrail`) | `GET /api/stocks`, `GET /api/stocks/{t}/memo`, `POST /api/stocks/{t}/analyze`, `GET /api/stocks/{t}/analyze/status`, `GET /api/stocks/{t}/memos`, `GET /api/stocks/{t}/memory`, `GET /api/quotes` | `/app/research` |
| `/dcf` | `pages/DCFLab.tsx` (+ `DCFVersionHistory`) | `GET /api/stocks`, `GET /api/dcf/{t}/default-assumptions`, `GET /api/dcf/{t}/consensus`, `GET /api/dcf/{t}/saved`, `POST /api/dcf/{t}`, `GET /api/admin/dcf-versions/{t}`, `GET /api/quotes` | `/app/dcf` |
