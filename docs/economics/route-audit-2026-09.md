...
| GET | `/api/admin/market-data/fmp-repull` | operations | DB-only re-pull ledgers | admin | admin | admin | The worker's FMP re-pull dry-run and execute ledgers: per-ticker quarantine ids and reasons, adoptions, restatements, entitlement denials, review list and result digest. No provider calls or writes. |
| POST | `/api/admin/market-data/fmp-repull/authorize` | operations | DB-only ledger row | admin | admin | admin | Owner authorization bound to the completed dry run's 64-hex result digest; creates the one-shot execute ledger the worker then runs. No provider call in the request; 404 before a dry run, 409 on a wrong digest, incomplete dry run or repeat. |
